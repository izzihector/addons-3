...
class PropertyBook(models.TransientModel):
    ...
    state = fields.Selection([('avl','Available'),('reserve','Reserve'),('occ','Occupied')], string="Status", default='avl')
    ...
    deposite_amount = fields.Float(string="Monthly Rent")
    total_deposite  = fields.Float("Total Rent")
    month = fields.Integer()
    #gym = fields.Boolean()
    #diesel = fields.Boolean()
    gym = fields.Selection([('inc','Included'),('not_inc','Not Included')])
    diesel = fields.Selection([('inc','Included'),('not_inc','Not Included')])


    @api.onchange('gym')
    def included_gym(self):
        property_obj = self.env['product.product'].search([('id','=',self.property_id.id)])
        if self.gym == 'inc':
            total = property_obj.club_fees + self.deposite
            self.deposite = total
        if self.gym == 'not_inc':
            total = self.deposite - property_obj.club_fees
            self.deposite = total

    @api.onchange('diesel')
    def included_diesel(self):
        property_obj = self.env['product.product'].search([('id','=',self.property_id.id)])
        if self.diesel == 'inc':
            total = property_obj.diesel_fees + self.deposite
            self.deposite = total

        if self.diesel == 'not_inc':
            total = self.deposite - property_obj.diesel_fees
            self.deposite = total
